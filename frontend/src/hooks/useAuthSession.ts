...
export interface UseAuthSessionResult {
  ...
  loginOpen: boolean;
  setLoginOpen: (value: boolean | ((prev: boolean) => boolean)) => void;
  authBusy: boolean;
  authError: string;
  setAuthError: (value: string | ((prev: string) => string)) => void;
  ...
  handleLoginSubmit: (credentials: LoginOptions) => Promise<void>;
  handleLogout: () => Promise<void>;
}

export function useAuthSession(): UseAuthSessionResult {
  const [auth, setAuth] = useState<AuthSession>(getInitialAuth);
  const [authReady, setAuthReady] = useState(false);
  const [loginOpen, setLoginOpen] = useState(false);
  const [authBusy, setAuthBusy] = useState(false);
  const [authError, setAuthError] = useState('');

  const can = useCallback((permission: string): boolean => hasPermission(auth, permission), [auth]);
  ...
  const requireLogin = useCallback(
    (action?: () => void, permission?: string): boolean => {
  ...
  const runProtectedMutation = useCallback(
    async (task: () => Promise<unknown>, fallbackMessage?: string, permission?: string): Promise<boolean> => {
      if (!(permission ? can(permission) : canEdit)) {
        setAuthError('');
        setLoginOpen(true);
        return false;
  ...
  const handleLoginSubmit = async (credentials: LoginOptions): Promise<void> => {
    if (authBusy) return;
    setAuthBusy(true);
    setAuthError('');
    try {
