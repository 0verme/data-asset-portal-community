...
export interface OperationResultOption {
  value: string;
  name: string;
}

export const OPERATION_RESULTS: readonly OperationResultOption[] = [
  { value: "success", name: "成功" },
  { value: "failed", name: "失败" },
] as const;

export interface MockOperationLogItem {
  id: number;
  userId: string;
  userName: string;
  deptName: string;
...
type SubjectTuple = [string, string, string, string];
...
export const OPERATION_LOGS: MockOperationLogItem[] = Array.from(
  { length: 20 },
  (_, index) => {
    ...
    const [moduleName, operationType, operationObject, operationDesc] = subject;
    return {
      id: 2000 + index,
      userId: `USR${String((index % 8) + 1).padStart(3, "0")}`,
      userName: "演示数据维护组",
      deptName: DEPT_NAMES[index % DEPT_NAMES.length] ?? "",
      moduleName,
      operationType,
      operationObject,
      operationDesc,
