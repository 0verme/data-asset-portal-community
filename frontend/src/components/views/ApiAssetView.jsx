...
function ApiList({ items, query, view, onChangeView, apiAsset }) {
  return <><div className="page-head"><div><div className="page-title"><span className="pt-code">API</span>API 资产</div><div className="page-sub">共 <b>{items.length}</b> 个 API，可按请求方式、下游调用系统和状态筛选。</div></div><div className="head-actions"><ViewModeSwitcher value={view} onChange={onChangeView} /><button className="btn primary" onClick={apiAsset.create}>新增 API</button></div></div>{!items.length ? <ApiEmptyState query={query} /> : view === "list" ? <div className="tbl-wrap"><table className="dt mobile-card-table"><thead><tr><th>API</th><th>请求</th><th>下游调用系统</th><th>负责人</th><th>状态</th><th>操作</th></tr></thead><tbody>{items.map((item) => <tr key={item.code}><td data-label=""><b>{item.name}</b><div className="mono">{item.code}</div></td><td data-label="请求"><span className="mono">{item.method} {item.path}</span></td><td data-label="下游系统">{systemLabel(item)}{systemCode(item)}</td><td data-label="负责人">{item.ownerName}</td><td data-label="状态"><StatusBadge status={item.status} /></td><td data-label="" className="mobile-card-actions"><RowActions disabled={apiAsset.pendingIds.includes(item.code)} onView={() => apiAsset.view(item.code)} onEdit={() => apiAsset.edit(item.code)} toggle={{ enabled: item.status === "enabled", label: item.name, onToggle: () => apiAsset.toggle(item) }} /></td></tr>)}</tbody></table></div> : view === "card" ? <ApiCards items={items} query={query} onView={apiAsset.view} onEdit={apiAsset.edit} onToggle={apiAsset.toggle} pendingIds={apiAsset.pendingIds} /> : <ApiGroups items={items} onView={apiAsset.view} />}</>;
}

export function ApiAssetView({ apiAsset, query, route, view, onChangeView }) {
