...
import { getScheduleSteps, type ScheduleStatus } from "./scheduleStepper.ts";

export { getScheduleSteps, nextUnload } from "./scheduleStepper.ts";

export interface DbBadgeProps {
  type: string;
}

export function DbBadge({ type }: DbBadgeProps) {
  return <span className="tag tag-neutral">{type}</span>;
}

const STATUS_SYMBOLS: Record<ScheduleStatus, ReactNode> = {
  completed: "✓",
  next: "●",
  pending: "○",
};

export interface ScheduleStepperProps {
  times: readonly unknown[] | null | undefined;
  muted?: boolean | undefined;
  now?: Date | undefined;
}

export function ScheduleStepper({ times, muted = false, now = new Date() }: ScheduleStepperProps) {
