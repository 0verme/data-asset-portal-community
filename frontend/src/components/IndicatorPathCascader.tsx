...
function buildColumns(
  ...
  const columns: IndicatorPathOptionNode[][] = [];
  ...
  while (currentOptions.length) {
    ...
    const activeNode = currentOptions.find(
...
export interface IndicatorPathChangeMeta {
  valuePath: string[];
  nodes: IndicatorPathOptionNode[];
  rootDimension: string;
  isLeaf: boolean;
}

export interface IndicatorPathCascaderProps {
  value?: string | undefined;
  options?: IndicatorPathOptionNode[] | undefined;
  ...
  onChange?:
    | ((value: string, meta: IndicatorPathChangeMeta) => void)
    | undefined;
}
...
export function IndicatorPathCascader({
  ...
  emptyText = "暂无可选路径",
  onChange,
}: IndicatorPathCascaderProps) {
  const rootRef = React.useRef<HTMLDivElement | null>(null);
  const selectedValuePath = React.useMemo(
    () => findIndicatorPathValuePath(value, options),
    [options, value],
  );
  const [open, setOpen] = React.useState(false);
  const [activeValues, setActiveValues] =
    React.useState<string[]>(selectedValuePath);
  ...
  const columns = React.useMemo(
    () => buildColumns(options, open ? activeValues : selectedValuePath),
    [activeValues, open, options, selectedValuePath],
  );
  const hasOptions = options.length > 0;

  const emitChange = (nextValuePath: string[]) => {
    ...
    onChange?.(nextPath, {
      ...
      isLeaf:
        nextNodes.length > 0 &&
        !nextNodes[nextNodes.length - 1]?.children?.length,
    });
  };

  const handleSelect = (level: number, option: IndicatorPathOptionNode) => {
  ...
  return (
    <div className="indicator-path-cascader" ref={rootRef}>
      ...
      >
        <span
          className={`indicator-path-trigger-text${displayValue ? "" : " placeholder"} mono`}
        >
        ...
        <div className="indicator-path-panel">
          <div className="indicator-path-panel-head">
            ...
            {selectedNodes.length ? (
              <span className="mono">
                {formatIndicatorPath(selectedValuePath, options)}
              </span>
            ) : null}
          </div>
          {loading ? (
            <div className="indicator-path-panel-empty">
              正在加载路径配置...
            </div>
          ) : null}
          {!loading && error ? (
            <div className="indicator-path-panel-empty error">{error}</div>
          ) : null}
          {!loading && !error && !hasOptions ? (
            <div className="indicator-path-panel-empty">{emptyText}</div>
          ) : null}
          {!loading && !error && hasOptions ? (
            <div className="indicator-path-columns">
              {columns.map((columnOptions, level) => (
                <div key={level} className="indicator-path-column">
                  <div className="indicator-path-column-title">
                    第 {level + 1} 级
                  </div>
                  <div className="indicator-path-option-list">
                    {columnOptions.map((option) => {
                      ...
                      const selected =
                        selectedValuePath[level] === option.value;
                      return (
                        <button
                          key={option.value}
                          type="button"
