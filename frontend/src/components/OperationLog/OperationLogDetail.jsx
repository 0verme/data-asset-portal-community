...
function Row({ label, children, mono, full }) {
  return (
    <div className={`oplog-detail-row${full ? " full" : ""}`}>
      ...
      <div className={`oplog-detail-value${mono ? " mono" : ""}`}>{children ?? "-"}</div>
    </div>
  );
}
 ...
 */
export function OperationLogDetail({ open, log, loading, error, onClose }) {
  React.useEffect(() => {
  ...
  return (
    <div className="confirm-mask system-modal-mask" onMouseDown={() => onClose?.()}>
      <div className="system-modal-card" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="system-modal-body">
          {loading ? (
            ...
            <div className="oplog-detail-grid">
              <Row label="操作时间" mono>{formatDateTime(log.createdAt)}</Row>
              <Row label="操作用户">{log.userName}</Row>
              <Row label="用户 ID" mono>{log.userId}</Row>
              <Row label="所属部门">{log.deptName}</Row>
              <Row label="模块名称">{log.moduleName}</Row>
              <Row label="操作类型">{log.operationType}</Row>
              <Row label="操作对象" mono full>{log.operationObject}</Row>
              <Row label="请求方式" mono>{log.requestMethod}</Row>
              <Row label="请求地址" mono full>{log.requestUrl}</Row>
              <Row label="请求参数" mono full>
                <pre className="oplog-detail-pre">{log.requestParams || "-"}</pre>
              </Row>
              <Row label="操作结果">
                <StatusBadge on={log.resultStatus === "success"} label={log.resultStatus === "success" ? "成功" : "失败"} />
              </Row>
              <Row label="耗时" mono>{formatCost(log.costTimeMs)}</Row>
              <Row label="错误信息" full>
                {log.errorMessage ? <span className="oplog-detail-error">{log.errorMessage}</span> : "-"}
              </Row>
              <Row label="IP 地址" mono>{log.ipAddress}</Row>
              <Row label="User-Agent" mono full>
                <pre className="oplog-detail-pre">{log.userAgent || "-"}</pre>
              </Row>
              <Row label="备注" full>{log.remark || "-"}</Row>
            </div>
          ) : null}
        </div>
      </div>
