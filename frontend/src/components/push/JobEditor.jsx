...
export function JobEditor({
  ...
  const defaultFreqType = freqTypeOptions[0]?.value || DEFAULT_FREQ_TYPE_OPTIONS[0].value;
  const defaultDelimiter = delimiterOptions[0]?.value || DEFAULT_DELIMITER_OPTIONS[0].value;
  const defaultEncoding = encodingOptions[0]?.value || DEFAULT_ENCODING_OPTIONS[0].value;
  const [form, setForm] = React.useState(() => ({
    ...
    freqType: initial?.freqType || defaultFreqType,
    freq: initial?.freq ?? defaultFreqParam(initial?.freqType || defaultFreqType),
    delimiter: initial?.delimiter || defaultDelimiter,
    encoding: initial?.encoding || defaultEncoding,
  ...
  const validation = touched ? validateJob(form, fields) : { errors: [], fieldErrors: {} };
  const delimiterSelectOptions = getLegacyAwareOptions(delimiterOptions, form.delimiter);
  const encodingSelectOptions = getLegacyAwareOptions(encodingOptions, form.encoding);
  const freqTypeSelectOptions = getLegacyAwareOptions(freqTypeOptions, form.freqType);
  ...
  const setSourceFileName = (value) => setForm((prev) => {
  ...
  const setFreqType = (value) => setForm((prev) => ({ ...prev, freqType: value, freq: defaultFreqParam(value) }));
  ...
  const deleteField = (fieldKey) => setFields((prev) => prev.filter((field) => field._key !== fieldKey));
    ...
    <div>
      ...
      <div className="form-card">
        ...
        <div className="form-grid">
          ...
          <div className="fl">
            <label>启用状态</label>
            <BinaryStatusToggle mode="enabled" value={form.enabled} onChange={(nextValue) => setValue("enabled", nextValue)} />
          </div>
        </div>
      </div>
          ...
          <table className="fields-edit mobile-edit-table">
            ...
            <tbody>
              {fields.map((field, index) => {
                const rowErrors = validation.fieldErrors[field._key] || {};
                  ...
                  <tr key={field._key}>
                    ...
                    <td data-label="">
                      <div className="row-tools">
                        ...
                        <button className="icon-btn" disabled={index === fields.length - 1} onClick={() => moveField(index, 1)} type="button">
                          <Icon name="down" size={14} />
                        </button>
                        <button className="icon-btn danger" disabled={fields.length === 1} onClick={() => deleteField(field._key)} type="button">
