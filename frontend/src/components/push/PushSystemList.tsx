import type { PublicPushSystem } from "../../api/push.ts";
import { getSystemBadgeText } from "../../utils/push.ts";
import { EmptyState, StatusBadge } from "../common/index.ts";
...
export function ProtocolTag({ protocol }: ProtocolTagProps) {
...
export interface PushSystemListProps {
  systems: readonly PublicPushSystem[];
  query: string;
  view: string;
            ...
            {systems.map((system) => {
              ...
              return (
                <tr className={isImportant ? "sys-row-important" : undefined} key={system.id} onClick={() => onOpen(system.id)}>
                  ...
                  {showContactDetails ? <>
                    <td data-label="下游对接人" style={{ color: "var(--ink-2)" }}>{downstreamContact || "未指定"}</td>
                    <td data-label="数据开发对接人" style={{ color: "var(--ink-2)" }}>{dataDeveloperContact || "未指定"}</td>
                  </> : null}
      ...
      {systems.map((system) => {
        ...
        return (
          ...
          >
            ...
            {dataDeveloperContact ? <div className="sys-conn">
              <span className="ck">数据开发对接</span>
              <span className="cv">{dataDeveloperContact}</span>
            </div> : null}
