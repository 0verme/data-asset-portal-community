...
export interface ParamDictPageProps {
  ...
  items: readonly ParamDictItemWithCategoryName[];
  selectedCategoryCode: string;
  query: string;
  canEdit: boolean;
  onPickCategory: (categoryCode: string) => void;
  onNew: () => void;
  onEdit: (item: ParamDictItemWithCategoryName) => void;
  onChangeStatus: (item: ParamDictItemWithCategoryName, status: string) => void;
}
...
export function ParamDictPage({
  ...
  const filteredItems = items.filter((item) => {
    if (selectedCategoryCode && item.categoryCode !== selectedCategoryCode)
      return false;
      ...
      <div className="page-head">
        <div>
          <div className="page-title">
            <span className="pt-code">DICT</span>参数字典
          </div>
          <div className="page-sub">
            当前分类 <b>{selectedCategory?.name || "未选择"}</b>
            {selectedCategory ? (
              <>
                ，共 <b>{filteredItems.length}</b> 条字典项
              </>
            ) : null}
            {query ? <>，匹配 “{query}”</> : null}
          </div>
            ...
            {categories.map((category) => (
              ...
              >
                <div>
                  ...
                  <div className="system-category-code mono">
                    {category.code}
                  </div>
                </div>
                <span className="system-category-count">
                  {category.count || 0}
                </span>
              </button>
            ))}
        ...
        <div className="system-panel system-panel-main">
          <div className="system-panel-head">
            <div>
              ...
              <div className="system-panel-sub">
                {selectedCategory?.desc || "请选择一个分类查看字典项。"}
              </div>
            </div>
          </div>
                    ...
                    <tr key={`${item.categoryCode}-${item.id}-${item.code}`}>
                      <td data-label="">
                        <span className="system-line-clamp">
                          <Highlight
                            text={item.categoryName || item.categoryCode}
                            q={query}
                          />
                        </span>
                      </td>
                      <td data-label="参数编码" className="mono">
                        <span className="system-line-clamp">
                          <Highlight text={item.code} q={query} />
                        </span>
                      </td>
                      <td data-label="参数名称">
                        <span className="system-line-clamp">
                      ...
                      <td data-label="参数值" className="mono">
                        <span className="system-line-clamp">
                          <Highlight text={item.value} q={query} />
                        </span>
                      </td>
                      <td data-label="状态">
                        <StatusBadge
                          status={item.status}
                          metaMap={PARAM_STATUS_META}
                        />
                      </td>
                      <td
                        data-label="更新时间"
                        className="mono param-updated-cell"
                      >
                        {formatDateTime(item.updatedAt)}
                      </td>
                      <td data-label="说明">
                        <span className="system-line-clamp">
                          <Highlight text={item.desc || "-"} q={query} />
                        </span>
                      </td>
                      <td
                        data-label=""
                        className="param-col-actions mobile-card-actions"
                      >
                        <RowActions
                          onEdit={() => onEdit(item)}
                          toggle={{
