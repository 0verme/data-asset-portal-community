...
export function ModuleSidebar({ module, context }: ModuleSidebarProps): ReactElement | null {
  const {
    apiAsset,
    apiAssetFilter,
    asset,
  ...
  if (module === "push") {
    return (
      <PushSidebar
        ...
        canEdit={canPermission("push:write")}
        pushRoute={pushRoute}
      />
    );
  }
  if (module === "indicator") {
    return (
      <IndicatorSidebar
        ...
        setIndicatorRoute={setIndicatorRoute}
        canEdit={canPermission("indicator:write")}
      />
    );
  }
  if (module === "report") {
    return (
      <ReportSidebar
        report={{ ...report, reportFilter, setReportFilter }}
        requireLogin={requireLogin}
        canEdit={canPermission("report:write")}
        setReportRoute={setReportRoute}
      />
    );
  }
  if (module === "apiAsset") {
    return (
      <ApiAssetSidebar
        ...
        setFilter={setApiAssetFilter}
        requireLogin={requireLogin}
        canEdit={canPermission("api_asset:write")}
      />
    );
  }
  if (module === "root") {
    return (
      <RootSidebar
        root={root}
        requireLogin={requireLogin}
        canEdit={canPermission("root:write")}
  ...
  if (module === "upstream") {
    return (
      <UpstreamSidebar
        ...
        statusOptions={statusOptions}
        requireLogin={requireLogin}
        canEdit={canPermission("upstream:write")}
  ...
  if (module === "lineage") {
    ...
    return (
      <div className="side-group">
        <div className="side-title">血缘范围</div>
        <div className="side-note">{note}</div>
        {details?.mode === "persistent" && details.snapshotAt ? (
          <div className="side-note">
            更新时间：{details.snapshotAt} · 节点：{details.nodeCount} · 关系：{details.edgeCount}
          </div>
        ) : null}
      </div>
    );
