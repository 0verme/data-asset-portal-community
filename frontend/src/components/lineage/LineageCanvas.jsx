...
export const LineageCanvas = React.forwardRef(function LineageCanvas({ graph, onSelect }, ref) {
  ...
  React.useImperativeHandle(ref, () => ({
    ...
    focusRoot: () => viewerRef.current?.fitNodes(rootNodeIds, ROOT_NEIGHBORHOOD_FIT_OPTIONS),
  }), [rootNodeIds]);

  return <div className="lineage-canvas-shell">
    <div className="lineage-view-tools" role="group" aria-label="血缘图视图操作">
      <button type="button" className="lineage-view-tool" onClick={() => viewerRef.current?.zoomBy(1 / 1.2)} aria-label="缩小" title="缩小">−</button>
      <button type="button" className="lineage-view-tool" onClick={() => viewerRef.current?.zoomBy(1.2)} aria-label="放大" title="放大">+</button>
      <button type="button" className="lineage-view-tool" onClick={() => viewerRef.current?.fitNodes(rootNodeIds, ROOT_NEIGHBORHOOD_FIT_OPTIONS)} aria-label="定位根节点邻域" title="定位根节点邻域">定位</button>
      <button type="button" className="lineage-view-tool" onClick={() => viewerRef.current?.fitView()} aria-label="适应全部" title="适应全部">适应画布</button>
    </div>
    <LineageViewerCanvas
      ref={viewerRef}
