...
const canvasPath = fileURLToPath(new URL("./lineage/LineageCanvas.jsx", import.meta.url));
...
test("lineage layout keeps the graph independent from the detail panel", async () => {
  ...
  assert.match(styles, /\.lineage-layout \{[^}]*grid-template-columns: minmax\(0, 1fr\) minmax\(300px, 340px\);[^}]*align-items: start;/);
  assert.match(styles, /\.lineage-canvas \{[^}]*display: flex;[^}]*height: clamp\(420px, 62vh, 720px\);[^}]*overflow: hidden;/);
  assert.match(styles, /\.lineage-canvas > lineage-viewer \{[^}]*flex: 1;[^}]*height: 100%;[^}]*min-height: 0;/);
  assert.match(styles, /\.lineage-view-tool \{[^}]*border-radius: var\(--radius-sm\);/);
});
...
test("initial lineage view uses the shared root-neighborhood fit", async () => {
  ...
  assert.match(canvas, /const ROOT_NEIGHBORHOOD_FIT_OPTIONS = \{ padding: 48, maxScale: 1 \}/);
  ...
  assert.doesNotMatch(canvas, /document\.createElement|replaceChildren|destroy\(\)/);
});
...
test("lineage filters wait for an explicit query", async () => {
  ...
  assert.match(page, /const \[pendingFilters, setPendingFilters\] = React\.useState/);
  ...
  assert.match(page, /if \(!nodeName\) \{[\s\S]*await loadGraph\(\{[\s\S]*\.\.\.pendingFilters/);
  assert.match(page, /onClick=\{\(\) => loadGraph\(route\)\}/);
});
...
test("lineage page supports table and detail views with snapshot diagnostics", async () => {
  const [page, styles] = await Promise.all([readFile(pagePath, "utf8"), readFile(stylesPath, "utf8")]);
