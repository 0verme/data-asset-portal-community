...
| `backend/alembic/versions/0002_*.py`–`0005_*.py` | Immutable forward revisions for existing databases and post-baseline changes | Runtime upgrade path | Yes | No | Alembic for SQLite/PostgreSQL/MySQL; DWS has no online Alembic path | migration lifecycle, CI integration and revision-head gates |
