...
COMMENT ON TABLE dwp.p_menu IS '数据资产门户系统菜单表';
COMMENT ON COLUMN dwp.p_menu.menu_id IS '主键ID';
COMMENT ON COLUMN dwp.p_menu.menu_code IS '菜单编码（模块标识）';
COMMENT ON COLUMN dwp.p_menu.menu_name IS '菜单名称';
COMMENT ON COLUMN dwp.p_menu.menu_icon IS '菜单图标';
COMMENT ON COLUMN dwp.p_menu.menu_path IS '路由路径';
COMMENT ON COLUMN dwp.p_menu.display_order IS '显示排序，数字越小越靠前';
COMMENT ON COLUMN dwp.p_menu.nav_placement IS '导航位置，primary/more';
COMMENT ON COLUMN dwp.p_menu.admin_only IS '是否仅管理员可见，Y/N';
COMMENT ON COLUMN dwp.p_menu.is_active IS '是否启用，Y/N';
COMMENT ON COLUMN dwp.p_menu.menu_desc IS '菜单说明';

CREATE UNIQUE INDEX idx_p_menu_uk_01
    ON dwp.p_menu (menu_code);

CREATE UNIQUE INDEX idx_p_menu_uk_02
    ON dwp.p_menu (menu_id);
