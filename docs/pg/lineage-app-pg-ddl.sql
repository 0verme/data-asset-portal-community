...
CREATE TABLE IF NOT EXISTS dwp.p_lineage_snapshot (
    ...
    status_code       VARCHAR(16)  NOT NULL CHECK (status_code IN ('ACTIVE', 'INACTIVE'))
);

CREATE TABLE IF NOT EXISTS dwp.p_lineage_node (
    snapshot_id    VARCHAR(128) NOT NULL REFERENCES dwp.p_lineage_snapshot(snapshot_id) ON DELETE CASCADE,
