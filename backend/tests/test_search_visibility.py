...
class SearchVisibilityTestCase(unittest.TestCase):
    ...
    def test_search_all_skips_disabled_menu_modules(self, mock_enabled_menu_codes):
        ...
        self.assertEqual(["asset", "system", "field", "root", "downstream"], [call.args[1]["type"] for call in mock_search_one.call_args_list])
...
class SearchRouteAliasTestCase(unittest.TestCase):
    def setUp(self):
        ...
        os.environ["FLASK_SECRET_KEY"] = "test-only-search-route-secret"
        self.app = create_app()
    ...
    @patch("backend.app.routes.search.search_provider.search")
    def test_route_accepts_module_query_alias(self, mock_search):
        mock_search.return_value = {"query": "首贷", "scope": "indicator", "groups": [], "total": 0}
...
class PortalVisibilityTestCase(unittest.TestCase):
    ...
    def test_portal_stats_skip_disabled_modules(self, mock_enabled_menu_codes):
        ...
        visible_modules = [config["module"] for config in service._visible_stat_configs()]
    ...
    def test_portal_stats_queries_share_service_transaction(self):
        ...
        with (
            patch("backend.app.services.portal_service.database_transaction", side_effect=transaction) as transaction_scope,
            patch("backend.app.services.portal_service.system_management_service.get_enabled_menu_codes", side_effect=enabled_menu_codes),
            patch("backend.app.services.portal_service.fetch_all", side_effect=fetch_stats),
        ):
