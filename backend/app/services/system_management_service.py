...
class SystemManagementService:
    ...
    def _next_pk(self, table, column):
    ...
    def _ensure_db_category_exists(self, category_code: str):
        rows = self._core_fetch(
            select(code_category.c.category_id).where(
                code_category.c.category_code == category_code
            )
        )
        if not rows:
    ...
    def get_param_dict_categories(self):
        statement = (
            ...
            .select_from(code_category.outerjoin(
                code_item, code_category.c.category_code == code_item.c.category_code
            ))
            .group_by(
                code_category.c.category_code,
                code_category.c.category_name,
        ...
        rows = self._core_fetch(statement)
        return [
            {
    ...
    def get_param_dicts(self, category_code: str | None = None):
        ...
        statement = (
            ...
            .select_from(code_item.join(
                code_category, code_category.c.category_code == code_item.c.category_code
            ))
            .where(*clauses)
    ...
    def _create_param_dict(self, payload):
        ...
        next_id = self._next_pk(code_item, code_item.c.item_id)
        self._execute([
    ...
    def _delete_param_dict(self, dict_id: str):
        ...
        if not rows:
            raise ParamDictNotFoundError(f"Parameter not found: {dict_id}")
        before = next((current for current in self.get_param_dicts() if current["id"] == str(item_id)), None)
        self._execute([delete(code_item).where(code_item.c.item_id == item_id)])
    ...
    def _create_menu(self, payload):
        ...
        next_id = self._next_pk(menu_table, menu_table.c.menu_id)
        self._execute([
    ...
    def _delete_menu(self, menu_id: str):
        ...
        if not before:
            raise MenuNotFoundError(f"Menu not found: {menu_id}")
        self._execute([delete(menu_table).where(menu_table.c.menu_id == item_id)])
        return before
