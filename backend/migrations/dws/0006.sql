-- Complete the Community runtime tables for GaussDB / DWS.
-- Mirrors postgresql/0006.sql with DWS dialect (BIGINT PKs, DISTRIBUTE BY).

CREATE SCHEMA IF NOT EXISTS dwp;

CREATE TABLE IF NOT EXISTS dwp.p_menu (
  menu_id BIGINT NOT NULL PRIMARY KEY, menu_code VARCHAR(64) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS dwp.p_code_category (
  category_id BIGINT NOT NULL PRIMARY KEY, category_code VARCHAR(64) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS dwp.p_code_item (
  item_id BIGINT NOT NULL PRIMARY KEY, category_code VARCHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS dwp.p_root_change_log (
  change_id BIGINT NOT NULL PRIMARY KEY, root_id BIGINT,
  root_abbr VARCHAR(64) NOT NULL, change_type VARCHAR(64) NOT NULL,
  change_summary VARCHAR(512), before_json TEXT, after_json TEXT,
...
CREATE TABLE IF NOT EXISTS dwp.p_indicator_change_log (
  change_id BIGINT NOT NULL PRIMARY KEY, indicator_pk BIGINT,
  indicator_id VARCHAR(64) NOT NULL, change_type VARCHAR(64) NOT NULL,
  change_summary VARCHAR(512), before_json TEXT, after_json TEXT,
  operator_name VARCHAR(64) NOT NULL DEFAULT 'system',
  change_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
)
