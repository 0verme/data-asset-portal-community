...
后续结构变更只新增 `backend/alembic/versions/` revision，不修改已发布 revision，不提供自动 downgrade。`0002_portable_asset_filter`、`0003_open_repository_modules`、`0004_metadata_ingestion_identity`、`0005_rbac_persistence`、`0006_field_mapping_upstream_id` 和 `0007_binary_status_contract` 是增量示例；`0004` 为 Asset source-scoped identity、Lineage import/content bookkeeping 提供 forward migration，并移除 legacy `table_name` global unique 约束；`0006` 将字段映射已有的 `upstream_system_id` 收口为 `p_upstream_system.system_pk` 外键，并对历史数据执行不猜测的 backfill；`0007` 将码值表可用状态从 legacy `active/draft/disabled` 收口为 `enabled/disabled`，并将历史 `active` / `draft` 分别迁移为 `enabled` / `disabled`。SQLite、PostgreSQL 与 MySQL 的 fresh/upgrade 路径都会从 baseline 升级到同一 head。DWS 目前保留离线基线验证与静态兼容验证；其 JDBC/provider 路径不宣称 online Alembic parity。
