-- Current Community schema baseline for MySQL 8.0.
CREATE TABLE IF NOT EXISTS p_system (
...
CREATE TABLE IF NOT EXISTS p_data_source (
...
CREATE TABLE IF NOT EXISTS p_api_asset (
...
CREATE TABLE IF NOT EXISTS p_api_param (
...
CREATE TABLE IF NOT EXISTS p_api_response_field (
...
CREATE TABLE IF NOT EXISTS p_api_relation (
...
ALTER TABLE p_api_asset ADD CONSTRAINT fk_p_api_asset_system
  FOREIGN KEY (system_id) REFERENCES p_system(system_id) ON DELETE RESTRICT;
CREATE INDEX idx_p_api_asset_filter ON p_api_asset(status_code, method_code, system_id);

CREATE TABLE IF NOT EXISTS p_field_mapping_table (
...
CREATE TABLE IF NOT EXISTS p_field_mapping_field (
...
ALTER TABLE p_field_mapping_table ADD CONSTRAINT fk_p_field_mapping_data_source
  FOREIGN KEY (data_source_id) REFERENCES p_data_source(source_id) ON DELETE RESTRICT;
ALTER TABLE p_field_mapping_field ADD CONSTRAINT fk_p_field_mapping_field_table
...
CREATE TABLE IF NOT EXISTS p_asset_domain (
...
CREATE TABLE IF NOT EXISTS p_asset_layer (
  layer_code VARCHAR(32) PRIMARY KEY, layer_name VARCHAR(128) NOT NULL,
  display_order INTEGER NOT NULL DEFAULT 0, is_active CHAR(1) NOT NULL DEFAULT 'Y',
  is_deleted CHAR(1) NOT NULL DEFAULT 'N'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
CREATE TABLE IF NOT EXISTS p_asset_table (
...
CREATE TABLE IF NOT EXISTS p_asset_change_log (
...
CREATE TABLE IF NOT EXISTS p_root_category (
...
CREATE TABLE IF NOT EXISTS p_root_item (
...
CREATE TABLE IF NOT EXISTS p_indicator_item (
...
CREATE TABLE IF NOT EXISTS p_indicator_path_config (
  ...
  path_level SMALLINT NOT NULL, full_path VARCHAR(512) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS p_operation_log (
...
CREATE TABLE IF NOT EXISTS p_code_category (
...
CREATE TABLE IF NOT EXISTS p_code_item (
...
CREATE TABLE IF NOT EXISTS p_root_change_log (
...
CREATE TABLE IF NOT EXISTS p_indicator_change_log (
...
CREATE TABLE IF NOT EXISTS p_upstream_system (
...
CREATE INDEX idx_p_upstream_system_data_source ON p_upstream_system (data_source_id);
CREATE INDEX idx_p_upstream_system_ix_01 ON p_upstream_system (status_code, db_type);
ALTER TABLE p_field_mapping_table ADD CONSTRAINT fk_p_field_mapping_table_upstream
  FOREIGN KEY (upstream_system_id) REFERENCES p_upstream_system(system_pk) ON DELETE RESTRICT;

CREATE TABLE IF NOT EXISTS p_upstream_unload_time (
...
CREATE INDEX idx_p_upstream_unload_time_ix_01 ON p_upstream_unload_time (system_pk, display_order);

CREATE TABLE IF NOT EXISTS p_upstream_change_log (
...
CREATE INDEX idx_p_upstream_change_log_ix_01 ON p_upstream_change_log (system_id, change_time);

CREATE TABLE IF NOT EXISTS p_push_system (
...
CREATE INDEX idx_p_push_system_master ON p_push_system (master_system_id);
CREATE INDEX idx_p_push_system_ix_01 ON p_push_system (status_code, protocol_type, dept_name);

CREATE TABLE IF NOT EXISTS p_push_job (
...
CREATE INDEX idx_p_push_job_ix_01 ON p_push_job (system_id, enabled_flag, freq_type);
CREATE INDEX idx_p_push_job_ix_02 ON p_push_job (system_id, is_deleted, job_code);

CREATE TABLE IF NOT EXISTS p_push_job_field (
...
CREATE INDEX idx_p_push_job_field_ix_01 ON p_push_job_field (job_id, field_order);

CREATE TABLE IF NOT EXISTS p_push_change_log (
    change_id BIGINT PRIMARY KEY,
    system_id BIGINT,
    job_id BIGINT,
...
CREATE INDEX idx_p_push_change_log_ix_01 ON p_push_change_log (system_id, change_time);
CREATE INDEX idx_p_push_change_log_ix_02 ON p_push_change_log (job_id, change_time);
CREATE INDEX idx_p_push_change_log_ix_03 ON p_push_change_log (object_type, object_code, change_time);

CREATE TABLE IF NOT EXISTS p_report_asset (
    ...
    related_tables_json TEXT NOT NULL,
    related_indicators_json TEXT NOT NULL,
    remark_desc TEXT,
    is_deleted CHAR(1) NOT NULL DEFAULT 'N',
    created_by VARCHAR(64) NOT NULL DEFAULT 'system',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_p_report_asset_ix_01 ON p_report_asset (status_code, report_type, domain_name);

CREATE TABLE IF NOT EXISTS p_manual_code_table (
...
CREATE TABLE IF NOT EXISTS p_lineage_snapshot (
...
CREATE TABLE IF NOT EXISTS p_lineage_node (
...
CREATE INDEX idx_p_lineage_node_lookup ON p_lineage_node (snapshot_id, kind_code, node_name);
CREATE TABLE IF NOT EXISTS p_lineage_edge (
    snapshot_id VARCHAR(128) NOT NULL,
...
CREATE INDEX idx_p_lineage_edge_source ON p_lineage_edge (snapshot_id, source_node_id);
CREATE INDEX idx_p_lineage_edge_target ON p_lineage_edge (snapshot_id, target_node_id);
